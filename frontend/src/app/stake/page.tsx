...
import { useNeuroVaultContract, StakerInfo, Goal as OnchainGoal } from "@/hooks/useNeuroVault";
  ...
  useEffect(() => {
    const loadContractData = async () => {
      ...
      try {
        ...
        setGoalList(
          activeGoals.map((goal: OnchainGoal) => ({
            id: goal.id,
            text: goal.text,
            proposer: "On-chain governance",
            status: "active",
            votesFor: 0,
            votesAgainst: 0,
            totalVoters: 0,
            createdAt: "On-chain active goal",
          }))
        );
      } catch (err) {
    ...
    loadContractData();
  }, [address, getTreasuryState, getStakerInfo, getTokenBalances, getActiveGoals]);
          ...
          {activeTab === "goals" && (
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between">
                ...
                <button
                  disabled
                  className="rounded-lg bg-zinc-800 text-zinc-500 px-4 py-2 text-xs font-medium flex items-center gap-1 cursor-not-allowed"
                >
                  <Plus className="w-3.5 h-3.5" /> Goal Writes Not Wired
                </button>
              </div>
              <div className="rounded-2xl border border-blue-500/30 bg-blue-500/10 p-4 text-sm text-zinc-300">
                This section now shows active goals from the vault contract. Goal proposal and direct goal voting are not implemented in the deployed contract yet, so writes remain unavailable here.
              </div>

              {/* Goal list */}
              {goalList.length === 0 ? (
                <div className="rounded-2xl border border-dashed border-zinc-700 bg-zinc-900/40 p-6 text-sm text-zinc-500">
                  No active goals found on-chain.
                </div>
              ) : goalList.map((goal) => {
                const style = goalStatusStyles[goal.status];
                const totalVotes = goal.votesFor + goal.votesAgainst;
                const approvalPct = totalVotes > 0 ? Math.round((goal.votesFor / totalVotes) * 100) : 0;
                const participationPct = goal.totalVoters > 0 ? Math.round((totalVotes / goal.totalVoters) * 100) : 0;
                ...
                return (
                  <div key={goal.id} className="rounded-2xl border border-zinc-800 bg-zinc-900/50 p-6 hover:border-zinc-700 transition-colors">
                    ...
                    <div className="rounded-lg border border-zinc-800 bg-zinc-950 p-3 text-xs text-zinc-500">
                      Goal governance is expressed through AI-generated proposals in the governance queue. This page is read-only until the contract exposes direct goal write actions.
                    </div>
                  </div>
                );
              })}
